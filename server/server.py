...
class Server(object):
    ...
    def check_proposed_early_stopping(self, t):
        """Compute growth rate and check early stopping condition.
        
        Growth rate is always computed and printed every round,
        but the counter is only updated before early stopping is triggered.
        """
        growth_rate = self.compute_proposed_metrics(t)

        # Only update counter if early stopping has not been triggered yet
        if self.proposed_early_stop_round is None and t > 1 and growth_rate != 0.0:
        ...
        # Always print growth rate regardless of early stopping status
        print(
            f"GR={growth_rate:.4f} | "
    ...
    def train(self):
        ...
        for t in progress_bar:
            ...
            # --proposed: compute and print every round (handled inside check_proposed_early_stopping)
            # --fast: skip growth rate computation entirely
            # Otherwise: compute metrics only (no early stopping check)
            ...
            # Validation early stopping (only if --validation is enabled)
            if self.args.validation:
                val_loss, val_acc, _, _, _ = self._validate_loader_(self.valloader)
                self.check_validation_early_stopping(t, val_loss, val_acc)

            # Fast mode: stop training once early stopping condition is met
            if self.args.fast:
                should_stop = False
